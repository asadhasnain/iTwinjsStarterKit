...
const queryElements = async (briefcaseDb: BriefcaseDb): Promise<Id64String[]> => {
  // This function is implemented to query elements from the briefcaseDb.
  ...
  while (await ecSqlReader.step()) {
   elementIds.push(ecSqlReader.current.ECInstanceId);
  }

  return elementIds; // No relation found
}
